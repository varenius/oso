...
/*  2015-01-26   lerner   Added 32 MHz to bw_dbbc  */
...
void get_tcal_fwhm1(device,tcal,fwhm,epoch,flux,corr,ssize,ierr,
...
float zero;
{
  ...
  *ierr=0;
  ...
  if(ifchain!=0) {
    ...
    if(shm_addr->lo.lo[ifchain-1]>15000
       && shm_addr->lo.lo[ifchain-1]<120000) {
      double cf;
      if(tempwx < -50.0 || tempwx > +50.0) {
	tempwx=20.0;
      }
{//ZZZ
  //char apa[500];
  //sprintf(apa, "inject \"APA:   shm_addr->lo.lo = %lf    center = %lf   fwhm = %lf", shm_addr->lo.lo, center, fwhm);
  //system(apa);
}
      cf=chopperwheel(load,sky,tempwx,zero);
      *tcal=cf*(tempwx+2.0+273.0);


/*
      printf(" cf %f tcal %f tempwx %f\n",cf,*tcal,tempwx);
      printf(" load %f sky %f zero %f\n",load,sky,zero);
*/
