...
C 
     ...
     &                 tpspc(j).lt.0.5.or.tpsor(j).lt.0.5) then
                  ...
                  call fc_get_tcal_fwhm1(lwhat(i),caltemps(j),fwhm,
     &                 epoch,flux,corr,ssize,ierr,
     &                 tpspc(j),tpsor(j),tempwx,tpzero(j))
               endif
            endif
         enddo
...
C        call fc_tsys_dbbc(ip,itpis_dbbc,ibuf,nch,nsub)
        call fs_get_tempwx(tempwx)
        call fc_tsys_dbbc1(ip,itpis_dbbc,ibuf,nch,nsub,tempwx)
        return
      else
