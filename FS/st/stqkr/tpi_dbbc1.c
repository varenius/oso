...
void tpi_dbbc1(ip,itpis_dbbc)                    /* sample tpi(s) */
long ip[5];                                     /* ipc array */
...
void tpput_dbbc1(ip,itpis_dbbc,isubin,ibuf,nch,ilen) /* put results of tpi */
long ip[5];                                    /* ipc array */
...
void tsys_dbbc1(ip,itpis_dbbc,ibuf,nch,itask,tempwx)
long ip[5];                                    /* ipc array */
int itpis_dbbc[MAX_DBBC_DET]; /* device selection array, see tpi_dbbc for details */
char *ibuf;              /* out array, formatted results placed here */
int *nch;                /* next available char index in ibuf on entry */
...
float tempwx;
{
  ...
  for (i=0;i<MAX_DBBC_DET;i++) {
	...
	float load, sky, zero;
	epoch=-1.0;
	load = shm_addr->tpical[ i];
	sky = shm_addr->tpi[ i];
	zero = 0;
	get_tcal_fwhm1(lwhat[i],&shm_addr->caltemps[i],&fwhm,
		       epoch,&dum, &dum,&dum,&ierr,load,sky,tempwx,zero);
	if(ierr!=0) {
	  ip[0]=ip[1]=0;
	  ip[2]=ierr;
	  return;
...
void cont_dbbc1(itpis_dbbc,dbbc_tpi,dbbc_tpical,samples,isubin,disp)
int itpis_dbbc[MAX_DBBC_DET]; /* device selection array, see tpi_dbbc for details */
