...
for rt in range(9):
    fbcmd("runtime={0}; net_port={1}{0}; mode=VDIF_8000-4096-8-2;".format(str(rt),np) + " datastream=add:{thread}:*;")
time.sleep(1)
# Start recording
for rt in range(1,9):
    fbcmd("runtime={0}; record=on:{1}".format(str(rt),scan_name))
time.sleep(1)
# Stop recording
for rt in range(1,9):
    fbcmd("runtime={0}; record=off".format(str(rt)))
sys.exit()
