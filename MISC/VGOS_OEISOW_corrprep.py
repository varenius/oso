...
def filltracks(of):
    of.write("$TRACKS;\n")
    of.write("  def VDIF_format;\n")
    of.write("    track_frame_format = VDIF;\n")
    of.write("  enddef;\n")

def fillmode(of):
    of.write("$MODE;\n")
    of.write("  def GEOSX.SX;\n")
    of.write("    ref $FREQ = VGOS_std:Oe:Ow;\n")
    of.write("    ref $FREQ = VGOS_std_Is:Is;\n")
    of.write("    ref $BBC = VGOS_std:Oe:Ow:Is;\n")
    of.write("    ref $IF = VGOS_std:Oe:Ow:Is;\n")
    of.write("    ref $PHASE_CAL_DETECT = Standard:Oe:Ow:Is;\n")
    of.write("    ref $TRACKS = VDIF_format:Oe:Ow:Is;\n")
    of.write("  enddef;\n")

def fillbbc(of):
    of.write("$BBC;\n")
    of.write("  def VGOS_std;\n")
...
def getfslogs(exp, year, doy):
    run(['scp','fulla:/usr2/log/'+exp+'oe.log', '.'])
    run(['scp','freja:/usr2/log/'+exp+'ow.log', '.'])
    run(['wget','ftp://ivs.bkg.bund.de/pub/vlbi/ivsdata/aux/'+year+'/'+exp+'/'+exp+'is.log', '-O',exp+'is.log'])
...
def fillEOP(of, year, doy):
    start = year + "-"+str(int(doy)-2)
...
for line in vex:
    ...
    if "end $TRACKS" in line:
        of.write(line)
        getfslogs(exp, year, doy)
        of.write("$CLOCK;\n")
        fillclock(of, exp+"oe.log")
        fillclock(of, exp+"ow.log")
        fillclock(of, exp+"is.log")
        fillEOP(of, year, doy)
    if keep:
        of.write(line)
    if "start = " in line and int(year) <0 and int(doy)<0:
        year = line.split()[2][0:4]
        doy = line.split()[2][5:8]
of.close()
